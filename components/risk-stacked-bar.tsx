...
const riskData = [
  { label: "High Risk", value: 23, color: "#ef4444", percentage: 18 },
  { label: "Medium Risk", value: 45, color: "#f97316", percentage: 35 },
  { label: "Low Risk", value: 60, color: "#22c55e", percentage: 47 },
]

export function RiskStackedBar() {
    ...
    <div className="space-y-6">
              ...
              <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-black/90 text-white text-sm px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-10">
                <div className="text-center">
                  ...
                  <div className="text-xs">
                    {item.value} shipments ({Math.round((item.value / total) * 100)}%)
                  </div>
                </div>
        ...
        {riskData.map((item, index) => (
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-slate-600">{item.value} shipments</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <style jsx>{`
        @keyframes slideIn {
          ...
          to {
            width: ${riskData.map((item) => item.percentage + "%")};
            opacity: 1;
          }
        }
      `}</style>
